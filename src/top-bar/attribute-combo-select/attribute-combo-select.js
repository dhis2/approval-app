...
const AttributeComboSelect = () => {
    ...
    const initializeSelectedAttributeCombo = (
        metadata,
        _attributeCombos,
        optionComboId
    ) => {
        ...
        return {
            value,
            attributeCombo: selectedAttrCombo,
            attributeOptionCombo: selectedAttrOptionCombo,
            isVisible,
        }
    }
    ...
    const handleSingleCategoryCombo = (_attributeCombos) => {
        ...
        if (
            ...
            const _attributeOptionCombo = findAttributeOptionCombo(
                singleCategoryCombo,
                categoryOptionMap
            )
            ...
            return {
                isVisible: false,
                value: i18n.t('1 selection'),
                attributeCombo: singleCategoryCombo,
    ...
    useEffect(() => {
        ...
        if (_attributeCombos.length === 0) {
            ...
            const result = initializeSelectedAttributeCombo(
                metadata,
                _attributeCombos,
        ...
        setShowed((prev) =>
            prev !== isShowAttributeComboVisible
                ? isShowAttributeComboVisible
                : prev
        )
        setAttrComboValue((prev) =>
            prev !== attributeComboValue ? attributeComboValue : prev
        )
    ...
    const onChangeCatCombo = (catComboId) => {
        ...
        if (
            catCombo.isDefault ||
            (catCombo.categories?.length === 1 &&
                catCombo.categories[0].categoryOptions.length === 1)
        ) {
